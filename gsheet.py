...
def write(date, item, expenses, category):
  ...
  try:
    ...
    result = (
        sheet.values()
        .update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range=RANGE_VALUE, valueInputOption='RAW', body={'values':[[date,item,expenses,category]]})
        .execute()
    )
    values = result.get("values", [])
